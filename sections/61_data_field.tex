%!TEX root = ../main.tex
\section{フィールドデータ}\label{sec:field-dataset}
...
UAVには、RTK-GPS(Real-Time Kinematic Global Positioning System)を搭載したDJI Mavic 3 Enterprise\cite{DJI-Mavic-3-Enterprise}を使用した。
搭載カメラは4/3型CMOSセンサー(有効画素数20MP)を搭載し、最大$5280 \times 3956$ピクセルの高解像度RGB画像を取得可能である
...
高度は地上高(AGL: Above Ground Level)\qty{12}{m}という低高度で実施し、高空間分解能の画像を確保した。
(ただし、後述するワークフロー(\cref{sec:workflow})ではSfM処理は最大解像度で行ったが、RA-GSでは1/4にダウンサンプリングした画像を用いた。)
