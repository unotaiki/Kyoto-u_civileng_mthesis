%!TEX root = ../main.tex
\section{実環境データ}\label{sec:field-dataset}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{1.0\linewidth}
    \centering
    \includegraphics[height=10cm, keepaspectratio]{figure/60_data/area1_label.png}
    \caption{調査地域の概要。}
    \label{fig:area1_map}
  \end{subfigure}
...
\subsection{調査地域}\label{subsec:study-area}
...
実世界における手法の検証には、京都府宇治市を流れる宇治川中流域にて取得したデータセットを使用した。
データの取得は2025年11月15日の午前9:00から10:30にかけてArea1を対象に現地調査により取得した(\cref{fig:study-area})。
...
撮影時の気象条件は晴れであり、Area 1観測時における気温は12.8$^\circ$Cから16.3$^\circ$C、風速は0.5から0.9 m/sと極めて穏やかで、UAVおよび光学センサーによる水底観測に最適な環境であった。
% \url{https://www.data.jma.go.jp/stats/etrn/view/10min_s1.php?prec_no=61&block_no=47759&year=2025&month=11&day=15&view=}
本調査では、流速が速い本流ではなく、流れから分派し水が滞留している副次的な水路を対象とした。
この水域は波が穏やかで透明度が非常に高く、水底がはっきり視認できる。
目視による推定最大水深は約1.2mであり、水面の一部には浮遊植物が確認された。
また、河床には藻類が生育しており、これにより水底が均質な砂礫ではなく視覚的に豊富なテクスチャを有している。
% これは後述する画像からの三次元再構成を有利にする条件である。
取得されたUAV画像の特徴的な例は、\cref{fig:area1_uav_images}に示す。

\subsection{使用機材}\label{subsec:uav-platform}
...
\begin{figure}[htbp]
  ...
  \begin{minipage}[t]{0.45\linewidth}
    ...
    \caption{使用したUAVプラットフォーム。
    DJI Mavic 3 Enterprise\cite{DJI-Mavic-3-Enterprise}。}
    \label{fig:mavic3e}
  \end{minipage}
  % \hspace{0.5cm}
  \hfill
...
UAVには、RTK-GPS(Real-Time Kinematic Global Positioning System)を搭載したDJI Mavic 3 Enterprise\cite{DJI-Mavic-3-Enterprise}を使用した(\cref{fig:mavic3e})。
RGBカメラは4/3型CMOSセンサー(有効画素数20MP)を搭載し、最大$5280 \times 3956$ピクセルの高解像度RGB画像を取得可能である
...
偏光フィルターの使用は取得画像の放射輝度値(Radiance)に影響を与え、放射輝度補正(\cref{sec:color-correction})と競合するが、
...
\subsection{データ取得}\label{subsec:data-acquisition}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    \quad Oblique Angle & 30$^\circ$--45$^\circ$ \\
...
高度は地上高(AGL: Above Ground Level)\qty{12}{m}という低高度で実施し、高空間分解能の画像を確保した(\cref{fig:area1_uav_images}参照)。
ただし、後述するワークフロー(\cref{chap:experiments})ではSfM処理は最大解像度で行ったが、RA-GSでは1/4にダウンサンプリングした画像を用いた。

ナディア画像は、オーバーラップ率を85\%、サイドラップ率を85\%に設定し、自動航行により108枚取得した。
% \note{OL,SL を覚えていない。リモコンから確認しないと。計算によるとだいたい90\%になる。85\%撮った気もする}
オブリーク画像については、特定の一点を中心とするマニュアル操作により、カメラのオフナディア角を主に約30$^\circ$--45$^\circ$ (一部、60$^\circ$を含む)で、円軌道を描くように対象地点を見下ろしながら、80枚を取得した。
カメラの角度および飛行経路の詳細は、\cref{fig:camera-path}に定性的に示す。
...
\begin{figure}[htbp]
  ...
  \caption{
    Area1におけるカメラパスと姿勢。
    SfMによるアライメント結果を表示している。
    NadirとObliqueの2種類のカメラアングルを組み合わせて撮影した。
    }
