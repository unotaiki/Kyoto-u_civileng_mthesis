...
時間の制約上、実装においては、\cref{sec:covariance-correction,sec:color-correction}でなく、前手法として提案したスケール補正\cref{sec:scale-correction}と不透明度補正\cref{sec:opacity-correction}を適用している。
...
さらに、3つの構成要素すべてを統合した本手法`RA-GS'が最も良い結果を達成し、この包括的なアプローチでは、\(\qty{0.011}{m}\)という最小のチャンファー距離と、\(94.00\%\)(\qty{10}{cm})および\(99.12\%\)(\qty{30}{cm})という最高のF1スコアを実現した。
