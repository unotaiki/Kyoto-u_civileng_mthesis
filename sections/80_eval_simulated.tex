...
\begin{table*}[htbp]
  \caption{
    外観に関する定量評価結果。
    提案手法の各構成要素を除去したアブレーション分析による比較を示す。
    各モデルは屈折を含むシーン画像から最適化している。
    % Render w/ Refraction: 屈折面を仮想的に配置した上で、再構成結果のシーンをレンダリングし、屈折による歪みを含む正解画像と比較する場合の新規視点からの外観一致度\\
    % Render w/o Refraction: 屈折面なしで最適化後のシーンをレンダリングし、歪みのない正解画像と比較する場合の新規視点からの外観一致度\\
  }
...
Correction Componetsの列で、位置補正、スケール補正、不透明度補正をアブレーションした場合の結果を各行に示している。
\kobayashi{補正のアブレーションであることの明記を追加}
