...
\begin{table*}[htbp]
  \caption{
    外観に関する定量評価結果。
    提案手法の各構成要素を除去したアブレーション分析による比較を示す。各モデルは屈折を含むシーン画像から学習されている。
  }
...
提案手法は、難易度の高い `w/o Refraction'のシナリオにおいても0.93以上のSSIMスコアを維持している。
...
\begin{figure}[htbp]
  ...
  \caption{
    ...
    補正を行わない場合は、視覚的に大きく歪んで推定されるが、位置補正を適用した提案手法ではどれでも高い視覚情報を復元できている。
    }
...
チャンファー距離は、2つの点群 $S_{gt}$ と $S_{est}$ における最近傍点間の平均二乗距離を測定するものであり、以下のように対称的に定義される:
...
さらに、3つの構成要素すべてを統合したRA-GSが最も良い結果を達成し、この包括的なアプローチでは、\(\qty{0.011}{m}\)という最小のチャンファー距離と、\(94.00\%\)(\qty{10}{cm})および\(99.12\%\)(\qty{30}{cm})という最高のF1スコアを実現した。
これらの結果は、最大高度に対する相対的な深度がおよそ、1:1という屈折の条件を受けやすいデータセット上で達成されたものであり、本手法の屈折モデル化が物理的に正確であることを示唆している。
前節で示した外観の向上に加え、本手法の複合的な補正戦略は、高忠実度な幾何形状の再構成においてより重要な役割を果たしていると言える。
