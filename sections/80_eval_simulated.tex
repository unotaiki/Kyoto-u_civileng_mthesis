...
位置補正の導入は最も実質的な改善をもたらし、CDを0.110 mまで大幅に減少させ、10 cm閾値でのF1スコアを76.51\%まで向上させた。
さらにスケール補正を加えることで形状が洗練され、CDは0.033 mに低下し、F1スコアは85.79\%に増加した。
スケール補正と不透明度補正はそれぞれ追加することで漸進的な改善をもたらすが、3つの構成要素すべてを統合した本手法`RA-GS'が相乗効果により最高の性能を達成している。
この包括的なアプローチにより、\(\qty{0.011}{m}\)という最小のチャンファー距離と、\(94.00\%\)(\qty{10}{cm})および\(99.12\%\)(\qty{30}{cm})という最高のF1スコアを実現した。
