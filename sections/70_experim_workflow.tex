...
本節では、主に実環境データを対象にした、屈折を考慮した水中三次元再構成のための完全なパイプラインとその実装を概説する。
...
\begin{sidewaysfigure}[p]
  ...
  \caption{
    ...
    RA-GSから得られる、屈折を除いたGaussianモデルから、TSDF Fusionによりメッシュ化された地形を抽出する。
    }
  \label{fig:workflow}
\end{sidewaysfigure}
...
本研究が対象とする空気中から水面下を対象としたシーンにおいては、光の屈折により、SfMが前提とする光の直進性が成立を阻害し、精度は悪化させる~\cite{she2024IROS_Refractive-COLMAP}。
...
UAV画像から水面や Distractor を抽出するマスク生成には、以下が実務における効率の観点から求められる。
...
Gaussian Splattingでは、SfMから得られた疎点群の位置をもとにGaussianを初期化するが、もしこれをこれをランダムな位置で初期化した場合、再構成の品質が低下することが示されている(\cref{fig:initialization-of-gaussians})。
...
もともと屈折の影響により優れた質の点群を生成できず、初期化のための点群であるから、時間効率を踏まえた低い品質のパラメータで行うことが可能である。
