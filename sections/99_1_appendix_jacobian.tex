%!TEX root = ../main.tex
\newpage
\section{見かけの位置へのヤコビ行列$J_{\mathrm{app}}$の導出}\label{sec:appendix-jacobian}

\cref{sec:position-correction}で述べた位置補正において、共分散補正(\cref{sec:covariance-correction})等で用いる変換の勾配には、真の位置$\bm{p}(r,z)$から見かけの位置$\bm{p}'(r',z')$へのヤコビ行列$J_{\mathrm{app}}$が必要となる。
本節では、$rz$平面における$2 \times 2$のヤコビ行列
...
\vspace{1em}
以上より、求めるヤコビ行列$J_{\mathrm{app}}$（$rz$平面における変換$\bm{p}=(r,z)^\top \mapsto \bm{p}'=(r',z')^\top$の勾配）は
...
\subsection{実装における水面･鉛直下における極限形}
