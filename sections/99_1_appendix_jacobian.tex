%!TEX root = ../main.tex
\newpage
\section{見かけの位置へのヤコビアン$J_{\mathrm{app}}$の導出}\label{sec:appendix-jacobian}

\cref{sec:position-correction}で述べた位置補正において、共分散補正(\cref{sec:covariance-correction})等で用いる変換の勾配には、真の位置$\bm{p}(r,z)$から見かけの位置$\bm{p}'(r',z')$へのヤコビアン行列$J_{\mathrm{app}}$が必要となる。
