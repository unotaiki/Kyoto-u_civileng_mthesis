...
その特徴は、フォトリアリスティックかつ高忠実度な三次元シーンのキャプチャ能力、高速な学習時間、そしてリアルタイムレンダリングにある。
...
その可能性はさらに広がり、衛星画像からの数値表層モデル(DSM)生成 \parencite{Aira2025CVPR_EOGS}、自動運転、そして水中三次元再構成 \parencite{li20243DV_watersplatting} といった様々な実世界アプリケーションへと拡張されている。
...
フォワードパスでは、三次元ガウス分布(3D Gaussians)の集合をラスタライズして画像を合成する。
各ガウス分布は、
中心位置 $\bm{p} \in \mathbb{R}^3$、
不透明度 $\alpha \in [0, 1]$、
球面調和関数(SH)によって表現される視点依存の色係数 $\bm{c}(\bm{p}, \bm{t}_i) \in \mathbb{R}^{3}$、
および三次元共分散行列 $\bm{\Sigma}^{\mathrm{3D}} \in \mathbb{R}^{3 \times 3}$ 
...
\end{align*} 
\cite{Zwicker2001_EWA-volume-splatting} の提案した射影手法に従い、カメラ空間における三次元共分散行列 $\Sigma_{cam}^{3D}$ は二次元画像平面へと射影される。
これは透視投影の一次近似(アフィン近似)のヤコビ行列 $\bm{J}$ を用いて行われ、二次元共分散行列 $\bm{\Sigma}^{\mathrm{2D}}$ が得られる：
...
このプロセスを通じて得られた三次元ガウス分布の集合は、三次元シーンを高忠実度で捉えることができる。
...
2DGSにおいて、シーンは配向された平らな二次元ガウス分布(Surfel)の集合として表現される。
...
ここで、$o_i$ は学習可能な不透明度パラメータ、$\bm{\Sigma}_{2D}$ は二次元スケーリングと回転行列から構成される局所的な分散行列である。
...
2DGSが「幾何学的に正確」であるための核心的な項である。各二次元ガウス円盤の固有法線 $\bm{n}_i$ を、色情報と同様の重みでブレンディングしたものである。
