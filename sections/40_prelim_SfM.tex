...
Structure from Motion (SfM) は、視点の異なる複数の画像群から、カメラの内部・外部パラメータとシーンの疎な(Sparse)3次元構造を同時に推定する手法である。
...
以下では、COLMAPをはじめとするSfMで用いられるIncremental SfMのパイプラインの理論的な流れを記述する。

\paragraph{カメラモデルと射影幾何}
SfMをはじめとする3D Visionでは、幾何学的記述に、3次元空間の点 $\bm{X} = [X, Y, Z]^\top \in \mathbb{R}^3$ を2次元画像平面上の点 $\bm{x} = [x, y]^\top \in \mathbb{R}^2$ へ投影する透視投影モデル(Pinhole Camera Model)が一般に用いられる。
...
\paragraph{特徴点マッチングと幾何学的検証}
SfMパイプラインの第一段階は、画像間の対応関係の構築である。
SIFT \cite{Lowe2004_SIFT} 等の記述子を用いて抽出された特徴点は、画像間でマッチングされ、同一の3次元点を指す2次元座標のペア $\{\bm{x}_i, \bm{x}_j\}$ が生成される。
...
RANSAC \cite{Fischler1987_RANSAC} アルゴリズムを用いてこの制約を満たすインライアのみを抽出することで、ロバストな対応関係が確立される。

\paragraph{増分的な再構成とバンドル調整}
初期の2画像から推定された構造に対し、Perspective-n-Point (PnP) アルゴリズムを用いて新たな画像のカメラ姿勢 $[\bm{R} | \bm{t}]$ を推定し、三角測量(Triangulation)によって新規の3次元点 $\bm{X}$ を生成するプロセスを繰り返すことで、シーン全体を段階的に復元する。
この過程で累積する推定誤差を最小化するため、最終段階あるいは各段階においてバンドル調整(Bundle Adjustment, BA)\cite{Triggs2000_BA} と呼ばれる大域的最適化が行われる。
BAは、全てのカメラパラメータ $\{\bm{P}_j\}$ と全ての3次元点座標 $\{\bm{X}_i\}$ を変数とし、観測された特徴点座標と再投影された座標とのユークリッド距離(再投影誤差)の総和を最小化する非線形最小二乗問題である：
...
以上のプロセスを経て出力される「高精度に推定されたカメラパラメータ」と「疎な3次元点群」は、後述するGaussian Splattingにおいて、ガウス分布の初期配置およびレンダリング時の視点情報の基礎として用いられる(\cref{fig:sfm_pipeline})。
