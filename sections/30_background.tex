...
\subsection{Airborne LiDAR Bathymetry}
...
\missingfigure{各手法のPros & Cons をTableにする。}


\subsection{Satellite Derived Multi-Spectral Bathymetry}
...
\subsection{人力測量}
日本で依然として主流である、人が川に入ってポールやレッド（測鉛）で測る手法、あるいはボートからのシングルビーム測深は、「危険（Danger）」、「疎（Sparse）」、「遅い（Slow）」。
点や線でのデータ取得となるため、断面と断面の間にある地形変化は完全に無視される。


\subsection{Photogrammetric Bathymetry}


\section{研究の応用先}\label{sec:application-of-research}
...
\subsubsection{研究の応用先の例: 聖牛による河川地形の時系列変化}
