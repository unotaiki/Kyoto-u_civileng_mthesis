...
これは、海図や河川図において、陸域の地形データと深部の水深データとの間に挟まれる浅水域には、有効な測量データが存在しないために白く描かれ、帯状をなすことに由来する(\cref{fig:white-ribbon-figure})。 
...
\begin{figure}[htbp]
  ...
  \caption{
    ...
    赤枠で囲われた白い領域は浅水域のため測量不能であり、空白で表示されている。
    }
...
河川における三次元情報は、その流下能力を決定する重要な要素であり、また、洪水予測などの河川マネジメントにおける基盤となる。
...
また、従来の水部計測は、測量員が有人船や直接川に入りポールで計測する手法に依存している(\cref{fig:manual-river-survey})。
これらは、水難事故のリスクを伴うだけでなく、測線維持のための慎重な操船や作業を要するため、広範囲のデータ取得において時間的効率性を欠く。
こうした背景と、少子高齢化による作業員の不足や、河川管理に面的な管理が求められるている状況を鑑みて、
...
この課題は、伝統的河川工法である「聖牛」の機能評価においても顕著に現れる。
聖牛は丸太と蛇籠を組み上げた構造物であり(\cref{fig:seigyu})、河川砂州部に設置することで流速･流域を制御し、災害の起こりにくい地形形成に寄与する。
近年では、「Nature-Based Solutions (NbS)」として再評価され、注目を集めている技術である。
砂州などの自然地形が形成されることで、これらは生物の生息地となり、生態系の多様性の基盤ともなる。
Fujiiら\cite{Fujii2024_seigyu}は、京都府木津川における6年間にわたるモニタリングを通じ、聖牛設置地域一帯を観測し、聖牛の設置によって地形が大きく変化することを明らかにした(\cref{fig:seigyu-morphological-change})。
同研究が示すような、砂州が単調な地形から水路や池が点在する複雑な地形へと遷移する過程を定量的に評価するためには、陸域と水域をシームレスに、かつ高解像度で捉える三次元計測技術が必要不可欠である。
...
\begin{figure}[htbp]
  ...
  \caption{
    ...
    丸太を組み合わせ、重しとして蛇籠を搭載した透過型の水制工である。
    }
    \label{fig:seigyu}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{聖牛による河川地形の時系列変化。
  \cite{Fujii2024_seigyu}より引用。\\
  増水時に氾濫した流れが水牛によって変えられることで、地形に変化が生じる長期的な過程を示している。
  }
...
この領域は、陸と水双方の環境要因が作用することで、高い生物多様性を有するエッジ効果(Edge Effect)が発現する場として知られる。
エコトーンの例･模式図を\cref{fig:ecotone-eg,fig:ecotone-illustration}に示す。

エコトーンの形状と生態学的プロセスの関係については、多くの研究がなされている。
例えば、Casaliniら\cite{Casalini2019_geo-ecological_patagonia_land}は乾燥地帯の植生分布が旧河道などの微地形に依存することを示し、Perryら\cite{Perry2018_geo-ecological_coral-reefs}は珊瑚礁の三次元構造が生物侵食速度や多様性に与える影響を報告している。
UAV空中写真測量を活用することで、サンゴ礁など浅水域の高解像度三次元モデルと色情報を同時に取得できる。こうしたデータは、底生生物群集の多様性や経時的な変化を定量評価する基盤情報として、広域マッピングや生息地モニタリングに有用である\cite{Joyce2018_Drone-marine-survey_and_how-to}。
これらの研究は、エコトーンの環境保全において、詳細な三次元地形情報が重要であることを示唆している。

浅水域エコトーンにおける生態系の豊かさは、太陽光の到達に由来する。
水生植物や藻類の基礎生産が行われる水深帯は「有光層(Photic Zone)」と呼ばれ、水底まで十分な光が届くことが生物生息の条件となる。
一方、受動的リモートセンシング(Passive Remote Sensing)である写真測量では、濁度や水深による制約が少なく、水底が確認可能な環境である必要がある。
従って、「生物多様性が高く計測ニーズのある浅水域」は、必然的に「光が水底まで達する写真測量が可能な場所」である可能性が高いと言える。
深すぎて光が届かない領域は、従来の音響測深が有効な領域であるが、浅水域エコトーンにおいては、光学的リモートセンシングは合理的なアプローチとなる。
また、エコトーンの理解には水域だけでなく連続する陸域の植生と地形の把握も同時に求められるため、水中のみを計測する無人水上艇による音響測深は適さない。
