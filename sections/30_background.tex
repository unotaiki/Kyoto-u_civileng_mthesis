...
\subsection{衛星由来水深 (Satellite Derived Bathymetry)}\label{subsec:bg_sdb}
\fix{SDBといえば、Spectrally derived bathymetryを指す。Spectralに焦点を当てた説明にする。}
衛星由来水深（Satellite Derived Bathymetry: SDB）は、LandsatやSentinel-2などのマルチスペクトル衛星画像を用い、太陽光の水中減衰率から水深を推定する手法である\cite{He2024_survey-shallow-bathymetry}。
受動的リモートセンシング（Passive Remote Sensing）であり、広域性や低コスト性において優れているが、河川環境への適用には以下の限界がある。

\begin{itemize}
  \item \textbf{空間解像度とミクセル問題}: Sentinel-2の解像度は\qty{10}{m}程度であり、中小河川においては1ピクセルの中に水域、陸域、植生が混在する「ミクセル(Mixed Pixel)」問題が発生し、精度が著しく低下する。
  \item \textbf{幾何学的制約の欠如}: SDBは輝度値に基づく放射量的な推定（Radiometric estimation）であり、幾何学的な三次元復元（Geometric reconstruction）ではない。そのため、精度確保には現地での実測水深データによるキャリブレーション（回帰モデルの補正）が不可欠であり、底質のアルベドの不均一性などの影響を直接的に受ける。
\end{itemize}
