...
有人航空機や大型ドローンに搭載する高出力LiDARは極めて高価(数千万円)である。
...
本手法は受動的リモートセンシング(Passive Remote Sensing)に分類され、広域かつ低コストに、そして最大20m程度の比較的大きい水深情報を取得できる利点がある。
...
\begin{itemize} 
  ...
  また、1ピクセル内に水域、陸域、河畔林が混在する「ミクセル(Mixed Pixel)」問題が発生し、水深推定を阻害する。 
\end{itemize}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{l>{\RaggedRight\arraybackslash}X>{\RaggedRight\arraybackslash}X}
    ...
    \end{itemize} &
    \begin{itemize}[leftmargin=1.2em, nosep, itemsep=0.2em]
      \item 浅水域では走査幅が狭く(\qty{3}{m}--\qty{8}{m}程度)、運用の時間的コストが高い
      \item 送受波器直下数十センチメートルは計測不可(リンギング制約)
      \item 陸域との連続的な地形計測が不可能
    \end{itemize} \\
    \midrule
    \textbf{ALB} & 
    \begin{itemize}[leftmargin=1.2em, nosep, itemsep=0.2em]
      \item 陸域と水域をシームレスに取得可能
      \item 広域カバレッジが可能
