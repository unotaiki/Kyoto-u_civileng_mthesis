...
3D Gaussian Splatting (3DGS) is a recent method that has achieved state-of-the-art results in novel view synthesis \cite{Kerbl2023ToG_3DGS}.
...
As an explicit 3D representation, 3DGS has been successfully applied to a wide range of tasks, including Visual-SLAM \cite{Yan2024CVPR_GS-SLAM,Zheng2025CVPR_WildGS-SLAM,Matsuki2024CVPR_GaussianSplattingSLAM}, human avatar creation \cite{Moreau2024CVPR_HumanGaussianSplatting,Shao2024CVPR_GaussianAvatar}, and feedforward 3D reconstruction \cite{Chen2024ECCV_MVSplatting}.
Its potential further extends to various real-world applications such as digital surface model (DSM) generation from satellite imagery \cite{Aira2025CVPR_EOGS}, UAV-based surveying, autonomous driving, and underwater 3D reconstruction \cite{li20243DV_watersplatting}.
...
\end{align*} 
Following the projection method from \cite{Zwicker2001_EWA-volume-splatting}, the 3D covariance in camera space, $\Sigma_{cam}^{3D}$ ,is then projected onto the 2D image plane. This is achieved using the Jacobian $\bm{J}$ of the affine approximation of the perspective projection, yielding a 2D covariance matrix $\bm{\Sigma}^{\mathrm{2D}}$:
...
During the backward pass, the optimization minimizes a photometric loss, which is a weighted sum of a $\mathcal{L}_1 (\Gamma, \Gamma_{gt})$ Loss and a D-SSIM loss $\mathcal{L}_\mathrm{D-SSIM} (\Gamma, \Gamma_{gt})$ \cite{Zhou2004_SSIM}:
...
\end{equation}
These formulations ensure that the entire pipeline is fully differentiable, allowing the parameters $\{\bm{p}, \bm{R}, \bm{s}, \bm{c}, \alpha\}$ to be optimized via gradient descent using the Adam optimizer \cite{Adam}.
